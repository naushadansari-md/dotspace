...
# Setup basic logging to help catch errors during development
...
class FocusWatcher:
    ...
    def get_valid_windows(self):
        ...
        if args.active_workspace:
            current_ws_node = focused.workspace()
            if current_ws_node:
                # Fixed: Use .num instead of trying to cast string names to int
                cur_ws = current_ws_node.num
            ...
            for item in self.i3.get_outputs():
                if hasattr(item, 'current_workspace') and item.current_workspace:
                    ws_list.append(str(item.current_workspace))
            for ws in tree.workspaces():
                if str(ws.num) in ws_list or ws.name in ws_list:
                    for w in ws.leaves():
                        if w.type and re.search(".*con$", w.type):
                            w_set.add(w.id)
            return w_set
        else:
    ...
    def on_window_focus(self, i3conn, event):
        if args.ignore_float and event.container and (
        ...
        def read(conn):
            ...
            if data == b"switch":
                with self.window_list_lock:
                    ...
                    # Fixed: Renamed iteration variable 'list' to 'win_id'
                    for win_id in current_list:
                        if win_id in windows:
                            self.window_index += 1
                            self.i3.command("[con_id=%s] focus" % win_id)
                            break
                        self.window_index += 1
                    else:
                        self.window_index = 0
    ...
    def run(self):
        t_i3 = threading.Thread(target=self.launch_i3, daemon=True)
        t_server = threading.Thread(target=self.launch_server, daemon=True)
        for t in (t_i3, t_server):
            t.start()
        
        # Keep main thread alive safely since sub-threads run in daemon mode
        t_server.join()
...
if __name__ == "__main__":
    parser = ArgumentParser(
        ...
        description="Cycle backwards through the history of focused windows (aka Alt-Tab).",
    )
    parser.add_argument("--history", dest="history", help="Maximum number of windows in history", type=int)
    parser.add_argument("--delay", dest="delay", help="Delay before updating history", type=float)
    parser.add_argument("--ignore-floating", dest="ignore_float", action="store_true", help="Ignore floating windows")
    parser.add_argument("--visible-workspaces", dest="visible_workspaces", action="store_true", help="Include visible workspaces only")
    parser.add_argument("--active-workspace", dest="active_workspace", action="store_true", help="Include active workspace only")
    parser.add_argument("--switch", dest="switch", action="store_true", help="Switch to previous window", default=False)
    parser.add_argument("--rswitch", dest="rswitch", action="store_true", help="Reverse switch", default=False)
    ...
    if args.switch or args.rswitch:
        try:
            ...
            logging.error("Could not connect to script daemon. Is the background server running?")
    else:
